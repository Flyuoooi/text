...
def do_train(
    ...
    for epoch in range(cfg.SOLVER.EPOCHS):
        ...
        for step, batch in pbar:
            ...
            if step < 3:
                print("mask_loss=", loss_items.get("mask_loss"))
                # 如果 outputs 里有 mask 预测/权重
                for k in ["mask", "mask_pred", "attn_mask", "mask_weights"]:
                    if k in outputs:
                        t = outputs[k]
                        print(k, t.mean().item(), t.min().item(), t.max().item())
...
def do_inference(cfg, model, dataset, val_loader, val_loader_same=None):
    ...
    if ds == "ltcc":
        ...
        evaluator_general = R1_mAP_eval(dataset.num_query_imgs, feat_norm=cfg.TEST.FEAT_NORM)
        eval_tasks = [
            ("Clothes-Changing", val_loader, evaluator_diff, True),
            ("General", val_loader, evaluator_general, False),
        ]
    elif ds == "prcc":
        evaluator_diff = R1_mAP_eval(dataset.num_query_imgs_diff, feat_norm=cfg.TEST.FEAT_NORM)
        evaluator_same = R1_mAP_eval(dataset.num_query_imgs_same, feat_norm=cfg.TEST.FEAT_NORM)
        eval_tasks = [
            ("Clothes-Changing", val_loader, evaluator_diff, False),
            ("Same-Clothes", val_loader_same, evaluator_same, False),
        ]
    else:
        evaluator = R1_mAP_eval(dataset.num_query_imgs, feat_norm=cfg.TEST.FEAT_NORM)
        eval_tasks = [
