...
class PromptLearner(nn.Module):
    def __init__(self, num_class, dataset_name, dtype, token_embedding, mask_prob=0.5):
        ...
        # We must ensure tokenized_prompts actually contains n_cls_ctx placeholder tokens,
        # otherwise token_suffix becomes mostly padding and EOT indexing breaks.
        n_ctx = 4
        n_cls_ctx = 16
    ...
    def forward(self, labels, mask_mode="random"):
        ...
        # 2) 根据类别取 cls_ctx（你现在的 cls_ctx 是 (N, n_cls_ctx, C)）
        ...
        # 4) 使用 cloth_direction 做“衣物方向”的投影 mask（你注释里的那段）
    ...
    def visualize_prompts(self):
        """
        可视化Prompt：
        - 每个 class 的最终 77 个 token
        - 可读字符串（用 SimpleTokenizer decode）
        - ctx 显示为 <CTX-c-i>
        """
        ...
        with torch.no_grad():
            ...
            prompts = torch.cat(
                [
                    prefix.expand(N, -1, -1),   # [N,5,512]
                    cls_ctx,                    # [N,4,512]
                    suffix.expand(N, -1, -1)    # [N,68,512]
                ],
                dim=1
            )  # [N,77,512]
            ...
            # decode 原始 token 模板（此为 “a photo of a person .”）
            ...
            for i in range(N):
                ...
                # 替换 learnable ctx（4 个 token）为显示标记
                # <CTX-i-0> <CTX-i-1> <CTX-i-2> <CTX-i-3>
    ...
    def visualize_masked_prompts(self, backbone_model, num_classes=5):
        """
        注意力引导的mask（backbone_model.apply_attn_guided_mask）
        """
        print("\n=== Attention-Guided Masked Prompt Visualization ===")
        print(f"num_classes = {num_classes}")
        print("ONLY cls_ctx will be masked; prefix/suffix are preserved.\n")

        # 准备一个 fake image（因为 attention mask 需要 image_feat）
        ...
        for cls_id in range(num_classes):
            ...
            # ① 生成原始 prompt token embedding（未 mask）
            ...
            # ③ 取 prefix / ctx / suffix 的范围
            prefix_len = self.token_prefix.shape[1]         # n_ctx + 1
            ctx_len = self.cls_ctx.shape[1]                 # n_cls_ctx
    ...
    def dump_coop_debug(self, labels, topk=8, max_labels=2, max_ctx_tokens=4):
        """返回一份“直观可读”的 CoOp 调试信息（尽量小、可打印）。
        learnable ctx 是 embedding 向量，不能直接 decode 成完整自然语言；
        这里用“最近邻词表投影”来近似观察它学到了什么语义。
        """
        import torch
        import torch.nn.functional as F

        if labels is None:
            return {"error": "labels is None"}
        ...
        # 2) ctx 统计：是否真的发生了学习（和初始化对比）
        with torch.no_grad():
            ctx_now = self.cls_ctx[labels_show].float()               # [B, ctx_len, C]
            ctx_init = self.cls_ctx_init[labels_show].float()         # [B, ctx_len, C]
            delta = ctx_now - ctx_init
            delta_norm_mean = delta.norm(dim=-1).mean().item()
            ctx_norm_mean = ctx_now.norm(dim=-1).mean().item()

            # cloth 相关性（你已经定义了 cloth_direction）
            cloth_cos = (F.normalize(ctx_now, dim=-1) * self.cloth_direction.float()).sum(dim=-1)  # [B, ctx_len]
            cloth_cos_max = cloth_cos.max(dim=1).values.detach().cpu().tolist()
            cloth_cos_mean = cloth_cos.mean(dim=1).detach().cpu().tolist()

        # 3) 最近邻 token 投影（用 token_embedding.weight 做词表）
        #    为了不太慢：只对 “ctx_mean” + “前 max_ctx_tokens 个 ctx token” 做 top-k
        with torch.no_grad():
            W = self.token_embedding.weight.float()                 # [V, C]
            Wn = F.normalize(W, dim=1)                              # [V, C]

            def topk_tokens(vec):  # vec: [C]
            ...
            for bi, lab in enumerate(labels_show.detach().cpu().tolist()):
                ctx_vecs = self.cls_ctx[lab].float()                # [ctx_len, C]

                # ctx 的平均向量（整体语义）
                ctx_mean = ctx_vecs.mean(dim=0)
                mean_top = topk_tokens(ctx_mean)

                # 前几个 ctx token 的语义（局部）
                per_token = []
        ...
        # 4) 一句“可读的 prompt 结构”（标记 learnable token）
...
class build_transformer(nn.Module):
    """
    入口形式：model = build_transformer(num_class, camera_num, view_num, cfg)

    当前功能：
    - image_encoder: 使用 CLIP 的 visual 模块 (ViT-B/16 / RN50)
    - BNNeck + classifier: 标准 ReID 头（feat & feat_proj 各一个）
    - PromptLearner + TextEncoder: 需要文本特征时调用 get_text=True
    - CAA + 残差校正：在 return_dict=True 时，额外返回 caa_t_to_v / r_v / v_corr
    """
    ...
    def __init__(self, num_classes, camera_num, view_num, cfg):
        ...
        #         )
        # Use cfg.MODEL.DEVICE_ID to select GPU; model.to(device) will move all params/buffers
        ...
        # 是否启用注意力引导文本 mask及比例（只作用在cls_ctx上）
        ...
        # 两对抗判别器
        self.disc_r_v = CAADiscriminator(dim=self.in_planes_proj)   # 让 r_v 变成“衣物残差”，不含身份
        self.disc_r_t = CAADiscriminator(dim=self.in_planes_proj)   # 可选：对文本残差做约束

    def apply_attn_guided_mask(self, prompts, image_feat):
        """
        prompts:   [B, L, C]  PromptLearner 生成的 token embedding
        image_feat:[B, C]    用来引导注意力的图像特征（建议用 feat_proj）
        只在 cls_ctx 部分做top-k mask，返回:
            prompts_masked: [B, L, C]
            attn_scores:    [B, L]  (可视化用)
        """
        B, L, C = prompts.shape
        ...
        img_norm = F.normalize(img_f, dim=-1, eps=1e-6).unsqueeze(1)     # [B,1,C]
        attn = (prompts_norm * img_norm).sum(dim=-1)                     # [B,L]

        # 只在 cls_ctx 范围内做 mask
        prefix_len = self.prompt_learner.token_prefix.shape[1]   # n_ctx+1
        ctx_len = self.prompt_learner.cls_ctx.shape[1]           # n_cls_ctx

        ctx_attn = attn[:, prefix_len: prefix_len + ctx_len]     # [B, ctx_len]

        # 至少 mask 1 个 token
        k = max(1, int(self.attn_mask_ratio * ctx_len))

        # 每个样本选出 top-k 注意力最高的 ctx token
        topk_idx = torch.topk(ctx_attn, k=k, dim=1).indices      # [B, k]
        # 转成全局下标
        topk_idx_global = topk_idx + prefix_len                  # [B, k]

        # 构造 mask 矩阵：1 表示要 mask
        mask = torch.zeros_like(attn, dtype=torch.bool)          # [B, L]
        mask.scatter_(1, topk_idx_global, True)                  # 在对应位置标 True

        # 扩展到 embedding 维度
        mask = mask.unsqueeze(-1).type_as(prompts)               # [B, L, 1]

        # 被 mask 的 token 置为 0（也可以换成某个 learnable [MASK] 向量）
        prompts_masked = prompts * (1.0 - mask)

        mask_bool = mask.squeeze(-1).bool()     # [B, L]
    ...
    def forward(
        ...
        **kwargs
        ...
        几种用法：
        1) 常规训练：
            (cls_scores, feats) = model(x, label=pid, cam_label=camid, view_label=viewid)
                - cls_scores = [cls_score, cls_score_proj]
                - feats      = [img_feature_last, img_feature, img_feature_proj]
        2) 只拿文本特征：
            text_feat = model(get_text=True, label=pid)
        3) 只拿图像特征：
            img_feat_proj = model(get_image=True, x=img)
        4) return_dict=True:
            outputs = model(x, label=pid, cam_label=camid, view_label=viewid, return_dict=True)
        """
        # if clothes_id is None and clothes_label is not None:
        #     clothes_id = clothes_label

        # 文本分支只编码文本
        if get_text:
            prompts = self.prompt_learner(label, mask_mode="none")
            tokenized = self.prompt_learner.tokenized_prompts.expand(prompts.size(0), -1).to(
                prompts.device
            )
        ...
        #         #  先得到 mask的 prompt.不在 PromptLearner里随机 mask，而是交给 attention-guided
        #         prompts = self.prompt_learner(label, mask_mode="none")   # [B, L, C]

        #         # 使用图像特征引导文本mask,feat_proj（已经 BNNeck 过，维度与 text 对齐）
        #         if self.use_attn_mask:
        #             # detach避免梯度直接从文本分支压到图像分支
        #             prompts, attn_scores = self.apply_attn_guided_mask(
        #                 prompts,
        #                 feat_proj.detach()
        #             )
        #             # 你如果想可视化 attn_scores，可以存到 outputs 里
        #             # outputs["text_attn"] = attn_scores
        ...
        #         # ===== 4) 原来的 CAA 残差循环保持不变 =====
        #         feat_v = feat_proj              # 视觉 proj 空间特征
        #         feat_t = text_features          # 文本特征

        #         t_to_v = self.caa_t2v(feat_t)   # 文本 → 视觉
        #         r_v = feat_v - t_to_v           # 残差，“衣物/模态差异”
        #         v_corr = feat_v - self.caa_gamma * r_v

        #         v_to_t = self.caa_v2t(feat_v)   # V -> T
        #         r_t = feat_t - v_to_t
        #         t_corr = feat_t - self.caa_gamma * r_t

        #         outputs["caa_t_to_v"] = t_to_v
        #         outputs["caa_r_v"] = r_v
        #         outputs["caa_v_corr"] = v_corr

        #         outputs["caa_v_to_t"] = v_to_t
        #         outputs["caa_r_t"] = r_t
        #         outputs["caa_t_corr"] = t_corr

        #         outputs["disc_r_v_feat"] = r_v   # 判别器用残差
        #         outputs["disc_r_t_feat"] = r_t
        #     print("=== DEBUG outputs ===")
        #     for k, v in outputs.items():
        #         if isinstance(v, torch.Tensor):
        #             print(f"{k}: Tensor, requires_grad={v.requires_grad}")
        #         else:
        #             print(f"{k}: NON-Tensor type: {type(v)}, value={v}")

        #     return outputs   # 只返回 feature
                # ===== 训练阶段 =====
        if not return_dict:
            # 原始模式（ReID baseline）
            return [cls_score, cls_score_proj], [
            ...
            #   基础输出（用于 ReID loss）
            outputs = {
                "logits": cls_score,          # 用于 ID Loss
                "features": img_feature,             # 用于 Triplet Loss
                "img_feat_proj": img_feature_proj,   # 用于 ITC（image）
            }
            ...
            if self.use_attn_mask:
                prompts, attn_scores, mask_bool = self.apply_attn_guided_mask(
                    prompts,
                    feat_proj.detach()   # 图像分支不被文本梯度攻击
                )

            # 文本特征（CLS 输出）
            text_features = self.text_encoder(prompts, tokenized)
            outputs["txt_feat_proj"] = text_features  # 用于 ITC 对比学习
            # ===== CoOp 调试：返回可直观看到的 prompt 信息（不会影响 loss）=====
            if debug:
                try:
            ...
            #   3) CAA loss（只告诉 loss_fn 用哪一个）
            # 使用 r_v 和 r_t 做对齐（按你论文设计）
            caa_loss = 0.5 * (
            ...
            #   4) Prompt Mask Regularization（可选）
            ...
            #   5) 可视化与 Debug 字段 —— 完全 detach，不参与梯度
            outputs["debug"] = {
    ...
    def get_text_embeddings(self, masked=False):
        """
        由 PromptLearner 生成 prompt → 由 TextEncoder 编码为 text embedding
        """
        with torch.no_grad():
            N = self.num_classes
            device = next(self.parameters()).device
            
            # 1) 得到未mask prompt（N,77,512）
            ...
            prompts = torch.cat([prefix, cls_ctx, suffix], dim=1)  # (N,77,512)

            # 2) 生成 masked prompt（可选）
            if masked:
                mask = torch.rand_like(prompts[:,:,0]) < self.prompt_learner.mask_prob
                prompts = prompts.clone()
                prompts[mask] = 0.0

            # 3) tokenized ids（用于 EOT 查找）
            tokenized = self.prompt_learner.tokenized_prompts.expand(N, -1)

            # 4) 调用 TextEncoder（正确位置）
            text_features = self.text_encoder(prompts, tokenized)

            return text_features.cpu().numpy()  # (N,512)
        
    @torch.no_grad()  # 配合attention后的mask可视化
    def dummy_forward_image(self, img):
        """
        用于可视化 attention mask 时获得 img_feature / img_feature_proj / feat / feat_proj
        （与 forward 里的图像分支保持一致）
        """
        # 和 forward 里的图像分支完全对齐
        if self.model_name == "RN50":
            ...
            # 可视化阶段不需要 cam/view 嵌入，直接设为 None
            cv_embed = None
            image_features_last, image_features, image_features_proj = self.image_encoder(
                img, cv_embed
            )
        ...
        # BNNeck，与 forward 中相同
        feat = self.bottleneck(img_feature)
        feat_proj = self.bottleneck_proj(img_feature_proj)

        # 这里返回 4 个量，和你在可视化函数中的解包保持一致
