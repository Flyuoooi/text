...
class PRCC(object):
    ...
    def _lookup_attr(self, img_abs_path, imgdir2attribute):
        ...
        if rel.startswith('rgb/'):
            rel2 = rel.replace('rgb/', '', 1)
            if rel2 in imgdir2attribute:
                return imgdir2attribute[rel2]
        rel3 = '/'.join(img_abs_path.split('/')[-4:])
        if rel3 in imgdir2attribute:
            return imgdir2attribute[rel3]
    ...
    def _process_dir_train(self, dir_path, aux_info):
        ...
        for pdir in pdirs:
            ...
            for img_dir in img_dirs:
                ...
                if aux_info:
                    # 5 元组：带属性
                    attr = self._lookup_attr(img_dir, imgdir2attribute)
                    dataset.append((img_dir, label, camid, clothes_id, attr))
                    images_info.append({'attributes': attr})
                else:
                    # 4 元组：完全无属性
    ...
    def _process_dir_test(self, test_path, aux_info):
            ...
            for pdir in pdirs:
                ...
                for img_dir in img_dirs:
                    ...
                    if cam == 'A':
                        ...
                        if aux_info:
                            attr = self._lookup_attr(img_dir, imgdir2attribute)
                            gallery_dataset.append((img_dir, pid, camid, clothes_id, attr))
                            images_info.append({'attributes': attr})
                        else:
                        ...
                        if aux_info:
                            attr = self._lookup_attr(img_dir, imgdir2attribute)
                            query_dataset_same_clothes.append((img_dir, pid, camid, clothes_id, attr))
                            images_info.append({'attributes': attr})
                        else:
                        ...
                        if aux_info:
                            attr = self._lookup_attr(img_dir, imgdir2attribute)
                            query_dataset_diff_clothes.append((img_dir, pid, camid, clothes_id, attr))
                            images_info.append({'attributes': attr})
                        else:
        ...
        pid2imgidx = {}
        if aux_info:
            for idx, (img_dir, pid, camid, clothes_id, attr) in enumerate(gallery_dataset):
...
if __name__ == '__main__':
    dataset=PRCC(root='/home/jwt/code/clip/data/Data/ReIDData',aux_info=True)
