...
def make_loss(cfg, num_classes, device):
    ...
    main_proj_align_weight = float(getattr(cfg.MODEL, "MAIN_PROJ_ALIGN_WEIGHT", 0.0))
    ...
    if main_proj_align_weight > 0:
        print(f"Using main/proj alignment loss, weight={main_proj_align_weight}")
    else:
        print("Main/proj alignment loss disabled (MAIN_PROJ_ALIGN_WEIGHT <= 0)")
    ...
    def loss_fn(outputs, pid):
        ...
        main_feat = outputs.get("features", None)
        proj_feat = outputs.get("features_proj", None)
        if main_proj_align_weight > 0 and main_feat is not None and proj_feat is not None:
            main_n = F.normalize(main_feat.float(), dim=1, eps=1e-6)
            proj_n = F.normalize(proj_feat.float(), dim=1, eps=1e-6)
            align_loss = (1.0 - (main_n * proj_n).sum(dim=1)).mean()
        else:
            align_loss = to_tensor(0.0)
        losses["main_proj_align_loss"] = align_loss
        ...
        total_loss = (
            ...
            + tri_proj_weight * tri_loss_proj
             + main_proj_align_weight * align_loss
            + itc_weight * itc
