...
class BackgroundGenerator(threading.Thread):
    ...
    """
    # 利用 threading 和 queue 提前从 generator 拉取数据
    # 标准的后台预取
...
class DataLoaderX(DataLoader):
    ...
    # 1. 自动检测 Distributed (DDP) 环境
    # 2. 创建 CUDA Stream 实现异步传输
    # 3. 使用 BackgroundGenerator 包装迭代器
    # 4. 在 __next__ 中实现非阻塞的 .to(device) 操作
