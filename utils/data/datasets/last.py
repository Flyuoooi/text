...
class LaST(object):
    ...
    def __init__(self, root='data', aux_info=False, meta_dir='PAR_PETA_105.txt',meta_dims=105, **kwargs):
        ...
        train, num_train_pids = self._process_dir(
            self.train_dir, pid2label=pid2label, clothes2label=clothes2label,
            relabel=True, aux_info=self.aux_info
        )
        val_query, num_val_query_pids = self._process_dir(
            self.val_query_dir, relabel=False, aux_info=self.aux_info
        )
        val_gallery, num_val_gallery_pids = self._process_dir(
            self.val_gallery_dir, relabel=False, recam=len(val_query), aux_info=self.aux_info
        )
        test_query, num_test_query_pids = self._process_dir(
            self.test_query_dir, relabel=False, aux_info=self.aux_info
        )
        test_gallery, num_test_gallery_pids = self._process_dir(
            self.test_gallery_dir, relabel=False, recam=len(test_query), aux_info=self.aux_info
        )
    ...
    def _process_dir(self, dir_path, pid2label=None, clothes2label=None,
                     relabel=False, recam=0, aux_info=False):
        ...
        for ii, img_path in enumerate(img_paths):
            ...
            if aux_info:
                relative_path = img_path.replace(self.dataset_dir + '/', '')
                if relative_path in imgdir2attribute:
                    attr = imgdir2attribute[relative_path]
                else:
                    logging.warning(f"[WARN] Attribute not found for {relative_path}, using zeros.")
                    attr = [0 for _ in range(self.meta_dims)]
                dataset.append((img_path, pid, camid, clothes_id, attr))
            else:
...
if __name__ == '__main__':
    dataset = LaST('/home/jwt/code/MADE-master/data/Data/ReIDData')
